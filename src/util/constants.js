import treze from "../images/logo-treze-de-campina-grande-256.png";
import serrabranca from "../images/logo-serra-branca.png";
import botafogopb from "../images/logo-botafogo-pb.png";
import sousa from "../images/logo-sousa-pb-256.png";
import flamengo from "../images/logo-flamengo-256.png";
import fluminense from "../images/logo-fluminense-256.png";
import vasco from "../images/logo-vasco-da-gama-256.png";
import botafogo from "../images/logo-botafogo-256.png";
import novaiguacu from "../images/logo-nova-iguacu-256.png";
import palmeiras from "../images/logo-palmeiras-256.png";
import corinthians from "../images/logo-corinthians-256.png";
import santos from "../images/logo-santos-256.png";
import saopaulo from "../images/logo-sao-paulo-256.png";
import bragantino from "../images/logo-red-bull-bragantino-256.png";
import paulistao2024 from "../images/Paulistão_Sicredi_2024_logo.png";
import carioca2024 from "../images/campeonato-carioca-2024.png";
import bayern from "../images/bayern-munchen-256.png";
import psg from "../images/paris-saint-germain-256.png";
import realmadrid from "../images/logo-real-madrid-escudo-256.png";
import manchestercity from "../images/manchester-city-football-club-256.png";
import barcelona from "../images/logo-barcelona-256.png";
import atleticomadrid from "../images/logo-atletico-madrid-256.png";
import dortmund from "../images/borussia-dortmund-256.png";
import arsenal from "../images/arsenal-football-club-256.png";
import tachira from "../images/deportivo-tachira-256.png";
import estudiantes from "../images/estudiantes-de-la-plata-256.png";
import alianzalima from "../images/logo-alianza-lima-256.png";
import atleticomg from "../images/logo-atletico-mineiro-256.png";
import barcelonasc from "../images/logo-barcelona-de-guayaquil-256.png";
import bolivar from "../images/logo-bolivar-256.png";
import caracas from "../images/logo-caracas-256.png";
import cerroporteno from "../images/logo-cerro-porteno-256.png";
import cobresal from "../images/logo-cobresal.png";
import colocolo from "../images/logo-colo-colo-256.png";
import gremio from "../images/logo-gremio-256.png";
import huachipato from "../images/logo-huachipato.png";
import idv from "../images/logo-independiente-del-valle-256.png";
import junior from "../images/logo-junior-barranquilla-256.png";
import ldu from "../images/logo-ldu-liga-de-quito-256.png";
import libertad from "../images/logo-libertad-256.png";
import liverpooluru from "../images/logo-liverpool-uru.png";
import millonarios from "../images/logo-millonarios.png";
import nacionaluru from "../images/logo-nacional-uruguai-256.png";
import palestino from "../images/logo-palestino-256.png";
import penarol from "../images/logo-penarol-256.png";
import riverplate from "../images/logo-river-plate-5.png";
import rosariocentral from "../images/logo-rosario-central-256.png";
import sanlorenzo from "../images/logo-san-lorenzo-256.png";
import talleres from "../images/logo-talleres-256.png";
import thestrongest from "../images/logo-the-strongest.png";
import universitario from "../images/logo-universitario.png";
import interlimeira from "../images/logo-inter-limeira.png";
import novorizontino from "../images/logo-novorizontino.png";
import campinense from "../images/logo-campinense-256.png";
import atleticopb from "../images/logo-atletico-pb.png";
import nacionalpb from "../images/logo-nacional-de-patos-pb.png";
import csp from "../images/logo-csp.png";
import pombal from "../images/logo-pombal.png";
import saopaulopb from "../images/logo-sao-paulo-crystal.png";
import madureira from "../images/logo-madureira-256.png";
import boavista from "../images/logo-boavista-rj-256.png";
import audaxrj from "../images/logo-audax.png";
import bangu from "../images/logo-bangu-256.png";
import sampaiorj from "../images/logo-sampaio-correa.png";
import voltaredonda from "../images/logo-volta-redonda-256.png";
import portuguesarj from "../images/logo-portuguesa-rj-256.png";
import guarani from "../images/logo-guarani-256.png";
import pontepreta from "../images/logo-ponte-preta-256.png";
import aguasanta from "../images/logo-agua-santa-256.png";
import santoandre from "../images/logo-santo-andre-256.png";
import saobernardo from "../images/logo-sao-bernardo-256.png";
import portuguesasp from "../images/logo-portuguesa-sp.png";
import mirassol from "../images/logo-mirassol-atualizado-256.png";
import ituano from "../images/logo-ituano-256.png";
import botafogosp from "../images/logo-botafogo-sp-256.png";
import realsociedad from "../images/logo-real-sociedad-256.png";
import copenhaghen from "../images/FC_Copenhagen_logo.png";
import lazio from "../images/societa-sportiva-lazio-256.png";
import rbleipzig from "../images/rb-leipzig-256.png";
import porto from "../images/logo-porto-256.png";
import napoli from "../images/societa-sportiva-calcio-napoli-256.png";
import psv from "../images/logo-psv-eindhoven-5.png";
import internazionale from "../images/football-club-internazionale-milano-256.png";
import atleticogo from "../images/logo-atletico-goianiense-com-estrela-256.png";
import athleticopr from "../images/logo-atletico-paranaense-256.png";
import bahia from "../images/logo-bahia-256.png";
import criciuma from "../images/logo-criciuma-5.png";
import cruzeiro from "../images/logo-cruzeiro-256.png";
import cuiaba from "../images/logo-cuiaba-256.png";
import fortaleza from "../images/logo-fortaleza-256.png";
import internacional from "../images/logo-internacional-256.png";
import juventude from "../images/logo-juventude-256.png";
import vitoria from "../images/logo-vitoria-256.png";
import brusque from '../images/logo-brusque-256.png';
import ceara from '../images/logo-ceara-256.png';
import crb from '../images/logo-crb-256.png';
import goias from '../images/logo-goias-esporte-clube-256.png';
import operariopr from '../images/logo-operario-pr-256.png';
import sampaio from '../images/logo-sampaio-correa-256.png';
import sport from '../images/logo-sport-recife-256.png';
import ypiranga from '../images/logo-ypiranga-de-erechim-256.png'
import aguiamaraba from '../images/logo-aguia-maraba.png';
import amazonas from '../images/logo-amazonas.png';
import americarn from '../images/logo-america-rn.png';

export const escudos = [
    { img: aguiamaraba, text: "Águia de Marabá"},
    { img: alianzalima, text: "Allianza Lima" },
    { img: amazonas, text: "Amazonas"},
    { img: americarn, text: "América-RN"},
    { img: arsenal, text: "Arsenal" },
    { img: athleticopr, text: "Athletico-PR" },
    { img: atleticogo, text: "Atlético-GO" },
    { img: atleticomg, text: "Atlético-MG" },
    { img: bahia, text: "Bahia" },
    { img: barcelonasc, text: "Barcelona-EQU" },
    { img: bayern, text: "Bayern Munchen" },
    { img: bolivar, text: "Bolivar" },
    { img: brusque, text: "Brusque"},
    { img: dortmund, text: "Borussia Dortmund" },
    { img: botafogo, text: "Botafogo-RJ" },
    { img: botafogosp, text: "Botafogo-SP" },
    { img: caracas, text: "Caracas" },
    { img: ceara, text: "Ceará"},
    { img: cerroporteno, text: "Cerro Porteño" },
    { img: cobresal, text: "Cobresal" },
    { img: colocolo, text: "Colo-Colo" },
    { img: corinthians, text: "Corinthians" },
    { img: crb, text: "CRB"},
    { img: criciuma, text: "Criciúma" },
    { img: cruzeiro, text: "Cruzeiro" },
    { img: cuiaba, text: "Cuiabá" },
    { img: tachira, text: "Deportivo Táchira" },
    { img: estudiantes, text: "Estudiantes" },
    { img: flamengo, text: "Flamengo" },
    { img: fluminense, text: "Fluminense" },
    { img: fortaleza, text: "Fortaleza" },
    { img: goias, text: "Goiás"},
    { img: gremio, text: "Grêmio" },
    { img: huachipato, text: "Huachipato-CHI" },
    { img: idv, text: "Independiente del Valle" },
    { img: internacional, text: "Internacional" },
    { img: junior, text: "Junior Barranquilla" },
    { img: juventude, text: "Juventude" },
    { img: ldu, text: "LDU" },
    { img: libertad, text: "Libertad" },
    { img: liverpooluru, text: "Liverpool-URU" },
    { img: millonarios, text: "Millonarios" },
    { img: nacionaluru, text: "Nacional-URU" },
    { img: operariopr, text: "Operário-PR"},
    { img: palestino, text: "Palestino" },
    { img: palmeiras, text: "Palmeiras" },
    { img: penarol, text: "Peñarol" },
    { img: psg, text: "PSG" },
    { img: bragantino, text: "RB Bragantino" },
    { img: realmadrid, text: "Real Madrid" },
    { img: riverplate, text: "River Plate" },
    { img: rosariocentral, text: "Rosario Central" },
    { img: sampaio, text: "Sampaio Corrêa"},
    { img: sanlorenzo, text: "San Lorenzo" },  
    { img: saopaulo, text: "São Paulo" },
    { img: sousa, text: "Sousa" },
    { img: sport, text: "Sport"},
    { img: talleres, text: "Talleres" },
    { img: thestrongest, text: "The Strongest" }, 
    { img: universitario, text: "Universitario" },
    { img: vasco, text: "Vasco" },
    { img: vitoria, text: "Vitória" },
    { img: ypiranga, text: "Ypiranga-RS"},
  ];
  
  export const logos = [
    { img: paulistao2024, text: "Paulistão 2024" },
    { img: carioca2024, text: "Carioca 2024" },
  ];
  
  export const inativos = [
    { img: aguasanta, text: "Água Santa" },
    { img: atleticopb, text: "Atlético-PB" },
    { img: atleticomadrid, text: "Atlético de Madrid" },
    { img: audaxrj, text: "Audax-RJ" },
    { img: bangu, text: "Bangu" },
    { img: barcelona, text: "Barcelona" },
    { img: boavista, text: "Boa Vista-RJ" },
    { img: botafogopb, text: "Botafogo-PB" },
    { img: campinense, text: "Campinense" },
    { img: csp, text: "CSP" },
    { img: copenhaghen, text: "FC Copenhagen" },
    { img: guarani, text: "Guarani" },
    { img: internazionale, text: "Internazionale" },
    { img: interlimeira, text: "Inter de Limeira" },
    { img: ituano, text: "Ituano" },
    { img: lazio, text: "Lazio" },
    { img: madureira, text: "Madureira" },
    { img: manchestercity, text: "Manchester City" },
    { img: mirassol, text: "Mirassol" },
    { img: nacionalpb, text: "Nacional de Patos" },
    { img: napoli, text: "Napoli" },
    { img: novaiguacu, text: "Nova Iguaçu" },
    { img: novorizontino, text: "Novorizontino" },
    { img: pombal, text: "Pombal" },
    { img: pontepreta, text: "Ponte Preta" },
    { img: porto, text: "Porto" },
    { img: portuguesarj, text: "Portuguesa-RJ" },
    { img: portuguesasp, text: "Portuguesa-SP" },
    { img: psv, text: "PSV" },
    { img: rbleipzig, text: "RB Leipzig" },
    { img: realsociedad, text: "Real Sociedad" },
    { img: santoandre, text: "Santo André" },
    { img: santos, text: "Santos" },
    { img: saobernardo, text: "São Bernardo" },
    { img: saopaulopb, text: "São Paulo Crystal" },
    { img: sampaiorj, text: "Sampaio Corrêa-RJ" },
    { img: serrabranca, text: "Serra Branca" },
    { img: treze, text: "Treze" },
    { img: voltaredonda, text: "Volta Redonda" },
  ];


export const imgDefault =
  "data:image/png;base64,";

